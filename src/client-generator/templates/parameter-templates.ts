...
import type { ParameterAnalysis, ParameterStructure } from "../models/parameter-models.js";
...
export function renderParameterInterface(analysis: ParameterAnalysis): string {
  ...
  if (structure.processed.queryParams.length > 0) {
    const queryProperties = analysis.queryProperties.map((prop) => {
      return `${prop.name}${prop.isRequired ? "" : "?"}: string`;
    });
 ...
 */
export function renderDestructuredParameters(analysis: ParameterAnalysis): string {
  ...
  // Body parameter
  if (structure.hasBody && structure.bodyTypeInfo) {
    const defaultValue = structure.bodyTypeInfo.isRequired ? "" : " = undefined";
