...
describe("strongly-typed parseApiResponseUnknownData", () => {
  describe("function generation", () => {
    ...
    it("should include proper overloads for with/without deserializerMap", () => {
      ...
      const overloadMatches = result.match(/export function parseApiResponseUnknownData</g);
  ...
  describe("type safety", () => {
    ...
    it("should have correct constraint on TSchemaMap", () => {
      ...
      expect(utilityCode).toContain("TSchemaMap extends Record<string, { safeParse:");
      expect(utilityCode).toContain("success: boolean; data?: unknown; error?: unknown");
    });
  });
});